...
{
    public abstract class ReadOnlyArchivableService<T, TSummary> : IReadService<TSummary> 
        where T: class, IEntity, IArchivable
        where TSummary: class, ISummary
    {
        protected readonly AppDbContext _context;
        private EntityCheckHelper _entityCheckHelper;

        protected ReadOnlyArchivableService(AppDbContext context) 
        {
            _context = context;
        ...
        {
            var entity = await _entityCheckHelper.EnsureExistsAndActive<T>(id);
